...
EagerContext::EagerContext(
    ...
    : ImmediateExecutionContext(kEager),
      ...
      use_send_tensor_rpc_(false),//BT性能
  ...
  runner_ = [this](std::function<void()> closure) {//BT多线程
  ...
  global_rendezvous_for_functions_ =
      core::RefCountPtr<Rendezvous>(CreateRendezvous(-1/*step_id*/));//BT通信
}
...
Status EagerContext::SelectDevice(DeviceNameUtils::ParsedName preferred,
                                  const NodeDef& ndef, Device** out) const {//BT设备
  DCHECK(out != nullptr);
