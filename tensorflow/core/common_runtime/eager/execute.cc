...
Status GetOrCreateKernelAndDevice(
  ...
  bool reuse_rendezvous_for_functions_original_value =
      ctx.GetReuseRendezvousForFunctions();
  // When running in eager_op_as_function mode Send/Recv ops need to be
  // placed on the same rendezvous to match the behaviour of eager mode.//BT自定函 BT通信
  bool reuse_rendezvous_for_functions =
      (ctx.RunEagerOpAsFunction() && !op->is_function()) ||
  ...
  if (kernel == nullptr) {//BTBT 若从cache中找不到对应kernel则创建一个
    ...
    if (op->is_function()) {//BTBT ??? 不确定是不是处理@tf.fn的场景?
      ...
      GetFuncAttr(op, ctx, kOutputsOnOpDevice, &function_outputs_on_op_device)//BTBT ??? function_outputs_on_op_device 是干啥的?
          .IgnoreError();
    }
    ...
    if (run_function_with_flr) {                                             //BTBT 如果是tf.fn则走if块???matmul不用tf.fn()也走此程序块了呀?
      ...
      ctx.SetReuseRendezvousForFunctions(reuse_rendezvous_for_functions);//BT通信
    ...
    if (op->is_function()) {
      ...
      if (KernelCacheEnabled(*op_def)) {
        ctx.AddKernelToCache(cache_key, kernel.get()); //BTBT 对于普通op或XLA op,除了dataset op其它都可cache
      }
    }
  }
